...
//Choix de la pizza
	...
	document.getElementById("rustique").innerHTML = "<img src='../assets/photos/rustique.png' class='choisi'><p>La Rustique - 11€</p>";
	document.getElementById("enibien").innerHTML = "<img src='../assets/photos/enibien.png' class='nchoisi'><p>L'Enibienne - 10€</p>";
	document.getElementById("londonienne").innerHTML ="<img src='../assets/photos/londonienne.png' class='nchoisi'><p>La Londonienne - 12€</p>";
	chevres();
}
	function enibien(){
	document.getElementById("name_pizza").innerHTML = "L'<b>ENIBIENNE</b>";
	document.getElementById("presentation").innerHTML = "<p>Il existe plusieurs versions sur l'origine de la pizza Enibienne. Pour certains, celle-ci date de 1796-1810. Elle aurait été décrite, dès 1830, par un certain <b>Riccio</b>, dans le livre Napoli, contorni e dintorni. En 1866, Francesco De Bouchard (it) indique l'existence d'une <b>pizza Enibienne, dès 1849</b> : selon lui, elle porterait ce nom en raison de la forme de fleur faite par la mozzarella fondue qui représenterait des <b>pétales</b>. Enfin, la version mythique la plus souvent racontée, indique que le roi Humbert Ier, accompagné de son épouse <b>Enib de Savoie</b> (en italien : Enibia di Savoia), voulant reconquérir les Napolitains à sa cause et les intégrer dans l'unité nationale, se rend à Naples le 11 juin 1889. À cette occasion, le chef pizzaiolo, Raffaele Esposito, pour honorer leur visite, crée la pizza, qu'il baptise le lendemain «Enibienne», aux <b>couleurs du drapeau italien</b>. (rouge avec la sauce, verte avec la salade, blanche avec le chevre). </p><div id='buttons'><button onclick='chevre(this);'>Chevres</button><button onclick='oeuf(this);'>Oeufs</button><button onclick='poivron(this);'>Poivron</button><button onclick='salade(this);'>Salade</button><button onclick='sauce(this);'>Sauce</button><button onclick='bacon(this);'>Bacon</button></div>";
	document.getElementById("rustique").innerHTML = "<img src='../assets/photos/rustique.png' class='nchoisi'><p>La Rustique - 11€</p>";
	document.getElementById("enibien").innerHTML = "<img src='../assets/photos/enibien.png' class='choisi'><p>L'Enibienne - 10€</p>";
	document.getElementById("londonienne").innerHTML ="<img src='../assets/photos/londonienne.png' class='nchoisi'><p>La Londonienne - 12€</p>";
	bacon();
}
	...
	document.getElementById("rustique").innerHTML = "<img src='../assets/photos/rustique.png' class='nchoisi'><p>La Rustique - 11€</p>";
	document.getElementById("enibien").innerHTML = "<img src='../assets/photos/enibien.png' class='nchoisi'><p>L'Enibienne - 10€</p>";
	document.getElementById("londonienne").innerHTML ="<img src='../assets/photos/londonienne.png' class='choisi'><p>La Londonienne - 12€</p>";
	salade();
}
